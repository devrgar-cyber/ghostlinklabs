...
def test_generated_docs_contain_no_placeholder_language() -> None:
    targets = [
        ...
        Path("ghostlink/docs/runtime_adaptation.md"),
    ]
...
def test_protocol_sections_cover_kernel_payload() -> None:
    ...
    assert protocol["sandbox"]["capability_floor"] == [
        cap["cap"]
        for cap in kernel["sovereignty"]["capabilities"]
        if cap.get("default", False)
    ]
    assert any(profile["name"] == "SILENCE" for profile in protocol["sandbox"]["pipelines"])
    assert protocol["immersion"]["layers"] == kernel["ui"]["layers"]
    assert protocol["custody"]["denylist"] == kernel["sovereignty"]["denylist"]
...
def test_docs_reflect_kernel_content() -> None:
    ...
    runtime_doc = Path("ghostlink/docs/runtime_adaptation.md").read_text(encoding="utf-8")
    for pipeline in kernel["pipelines"]:
        assert pipeline["name"] in runtime_doc
        for stage in pipeline["multipaths"]:
            assert stage in runtime_doc
    for layer in kernel["ui"]["layers"]:
        assert layer in runtime_doc
    for capability in kernel["sovereignty"]["capabilities"]:
        assert capability["cap"] in runtime_doc
    for deny in kernel["sovereignty"].get("denylist", []):
        assert deny in runtime_doc


def test_sandbox_and_immersion_structures() -> None:
    kernel = ghostlink.load_kernel()
    sandbox = ghostlink.derive_sandbox_matrix(kernel)
    immersion = ghostlink.derive_immersive_ui(kernel)
    custody = ghostlink.derive_custody_manifest(kernel)

    assert sandbox["determinism"] == kernel["determinism"]
    assert len(sandbox["pipelines"]) == len(kernel["pipelines"])
    for pipeline, profile in zip(kernel["pipelines"], sandbox["pipelines"]):
        assert profile["name"] == pipeline["name"]
        assert profile["stage_count"] == len(pipeline["multipaths"])
        assert [stage["code"] for stage in profile["stages"]] == pipeline["multipaths"]

    assert immersion["layers"] == kernel["ui"]["layers"]
    assert immersion["drivers"] == kernel["ui"]["drivers"]
    assert any(route["name"] == "SILENCE" for route in immersion["quiet_routes"])

    assert custody["denylist"] == kernel["sovereignty"]["denylist"]
    assert custody["integrity"]["manifest"]["files"] == kernel["integrity"]["manifest"]["files"]
