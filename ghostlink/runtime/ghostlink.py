...
def derive_sandbox_matrix(kernel: dict[str, Any]) -> dict[str, Any]:
    """Project pipeline execution into a deterministic sandbox schedule."""
    ...
    pipeline_profiles: list[dict[str, Any]] = []
    for pipeline in kernel["pipelines"]:
        stages = [
            {"ordinal": index, "code": stage}
            for index, stage in enumerate(pipeline["multipaths"], start=1)
        ]

        if pipeline["action"] == "accelerate":
            pace = "accelerated"
        elif pipeline["name"].upper() in {"LOCK", "SEAL"}:
            pace = "governed"
        elif pipeline["name"].upper() == "SILENCE":
            pace = "muted"
        else:
            pace = "steady"

        pipeline_profiles.append(
            {
                "id": pipeline["id"],
                "name": pipeline["name"],
    ...
    return {
        "determinism": gather_determinism(kernel),
        "capability_floor": capability_floor,
        "pipelines": pipeline_profiles,
        "laws": [law["id"] for law in kernel["laws"]],
        "output_rules": [rule["id"] for rule in kernel["output_rules"]],
    }


def derive_immersive_ui(kernel: dict[str, Any]) -> dict[str, Any]:
    """Describe adaptive UI layers that remain unobtrusive."""

    layers = gather_ui_layers(kernel)
    drivers = gather_ui_drivers(kernel)
    function_register = gather_function_register(kernel)
    ...
    quiet_routes = [
        ...
        for pipeline in kernel["pipelines"]
        if pipeline["name"].upper() == "SILENCE"
    ]

    growth_tracks = [
        {
            "track": category,
            "milestones": list(entries),
            "count": len(entries),
        }
        for category, entries in function_register.items()
    ]
...
def derive_custody_manifest(kernel: dict[str, Any]) -> dict[str, Any]:
    """Summarize sovereignty and integrity expectations for user-held data."""

    sovereignty = gather_sovereignty(kernel)
    integrity = gather_integrity(kernel)

    baseline_caps = [entry["cap"] for entry in sovereignty["capabilities"]]

    return {
        "signature_required": sovereignty.get("signature_required", False),
        "denylist": list(sovereignty.get("denylist", [])),
...
def ghostlink_protocol(kernel: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "sandbox": derive_sandbox_matrix(payload),
        "immersion": derive_immersive_ui(payload),
        "custody": derive_custody_manifest(payload),
    }
...
SECTION_HANDLERS = {
    ...
    "sandbox": derive_sandbox_matrix,
    "immersion": derive_immersive_ui,
    "custody": derive_custody_manifest,
}
...
def _render_section(section: str, data: Any) -> str:
    ...
    if section == "sandbox":
        ...
        lines.extend(f"- {cap}" for cap in data["capability_floor"] or ["<none>"])
        lines.append("")
        lines.append("Pipeline Enclaves:")
        for profile in data["pipelines"]:
            lines.append(f"[{profile['id']}] {profile['name']} :: {profile['action']} ({profile['pace']})")
            for stage in profile["stages"]:
                lines.append(f"  - {stage['ordinal']:02d} :: {stage['code']}")
        lines.append("")
        lines.append("Determinism:")
        lines.extend(
            f"- {key} = {value}" for key, value in data["determinism"].items()
        )
        return NEWLINE.join(lines)
    if section == "immersion":
        lines = ["Layers:"]
        lines.extend(f"- {layer}" for layer in data["layers"])
        lines.append("")
        lines.append("Drivers:")
        for driver in data["drivers"]:
            descriptor = driver["desc"]
            details = []
            if "grid" in driver:
                details.append(f"grid={driver['grid']}")
            if "glyphs" in driver:
            ...
            lines.append(f"- {driver['name']} — {descriptor}{suffix}")
        if data["quiet_routes"]:
            lines.append("")
            lines.append("Quiet Routes:")
            for route in data["quiet_routes"]:
                lines.append(
                    f"- {route['name']} ({route['action']}) :: pipeline {route['id']}"
                )
        lines.append("")
        lines.append("Growth Tracks:")
        for track in data["growth_tracks"]:
            lines.append(f"[{track['track']}] ({track['count']})")
            for milestone in track["milestones"]:
                lines.append(f"  - {milestone}")
        return NEWLINE.join(lines)
    if section == "custody":
        lines = [f"Signature required: {str(data['signature_required']).lower()}"]
        lines.append("Denylist:")
        lines.extend(f"- {entry}" for entry in data["denylist"] or ["<none>"])
        lines.append("")
        lines.append("Capabilities:")
        lines.extend(f"- {entry}" for entry in data["capabilities"])
        lines.append("")
        lines.append("Integrity Manifest:")
        for item in data["integrity"]["manifest"]["files"]:
            lines.append(f"- {item['path']} :: {item['sha256']}")
        if data["integrity"]["policy"]:
            lines.append("")
            lines.append("Policy:")
            lines.extend(
                f"- {key} = {value}"
                for key, value in data["integrity"]["policy"].items()
            )
        return NEWLINE.join(lines)
