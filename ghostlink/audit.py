...
def audit_components(root_package: str = "ghostlink") -> tuple[Sequence[ComponentRecord], Sequence[AuditIssue]]:
    ...
    def _record_import_error(module_name: str, exc: Exception) -> None:
        severity: Literal["error", "warning"] = "error"
        reason = f"import failed: {exc.__class__.__name__}: {exc}"

        if isinstance(exc, ModuleNotFoundError):
            missing = getattr(exc, "name", None)
            if isinstance(missing, str) and not missing.startswith(root_package):
                severity = "warning"
                reason = f"optional dependency missing: {missing}"
